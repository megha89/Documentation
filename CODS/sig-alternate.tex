...
%\terms{Theory}
%
\keywords{OCR error classification, crowdsourced text correction, multi class SVM}
...
The same set of queries were applied on both the datasets, that is raw OCR and corrected OCR individually. For each of the of $23$ queries, we chose the most relevant document from the retrieved results and observed the rank of the most relevant document in both the set of results. The MRR for the raw OCR data was noted to be $0.352$ whereas for corrected data it was $0.629$. The higher value of MRR denotes that relevant results get higher priority in corrected corpus. The metric MRR was chosen because it did not require relevance feedback from the users as in precision/recall \cite{manning2008introduction}.
