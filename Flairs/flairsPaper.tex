...
We are performing automatic text correction by modelling it through a machine learning algorithm, called multi class Support Vector Machines \cite{multiclass}. In this formulation, multi class problem is posed as a constrained optimization problem with a quadratic objective function.
...
As can be seen in table ~\ref{table: error} , the error is minimum when the C has value (1 and 10) and when the kernel type is radial basis function (gaussian). But the time taken as shown in table ~\ref{table: runtime} to train the model for these parameters, when C is 1 is 16468 cpu sec and when C is 10 is 55308 cpu sec.
\subsection{Known Defects}
The \cite{multiclass} algorithm converges quickly for linear type kernel but the its performance on test set on poor. For non linear kernels, this algorithm does not scale well for large scale datasets but gives better performance than linear kernels.
