...
%Devaine et al. (CITE) study ...
 Data aggregation also known as data fusion is defined as set of methods for combining the data that comes from many sensor nodes into a set of meaningful information \cite{Heinzelman00energy}.
...
Load forecasting is extremely important for operation and planning of an electric utility company. It is usually divided into three categories, short-term forecasts \cite{Bakirtzis} \cite{Chen} \cite{Fan} \cite{Chow}, medium-term forecasts \cite{Falvo} \cite{samuel} \cite{bassi} and long-term forecasts \cite{Daneshi} \cite{Zhang}. Most of the load forecasting algorithms use statistical and artificial intelligence algorithms.
...
For automaton R, X$_{R}$ = \{2; $R_1$,$R_2$\}, X$_{0R}$ = \{$R_1$\}, A$_{R}$ = \{a,b\}, T$_{R}$ = \{$R_1$,a,$R_1$; $R_1$,b,$R_2$; $R_2$,a,$R_2$; $R_2$,b,$R_1$\} \\
For automaton S, X$_{S}$ = \{3; $S_1$,$S_2$,$S_3$\}, X$_{0S}$ = \{$S_1$\}, A$_{S}$ = \{a,b\}, T$_{S}$ = \{$S_1$,a,$S_1$; $S_1$,b,$S_2$; $S_2$,a,$S_2$; $S_2$,b,$S_3$; $S_3$,a,$S_3$; $S_3$,b,$S_1$\} \\
...
\nocite{Albert,wijaya2014consumer,Zhang,Daneshi,bassi,samuel,Falvo,Bakirtzis,Chen,Fan,Chow,DisaggregationHSMM,KolterJ12,KolterF11,BLTJ:BLTJ21650,Heinzelman00energy,Taylor,NYAS:NYAS5921,
